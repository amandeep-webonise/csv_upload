...
func (s *Srv) UploadFile(w *framework.Response, r *framework.Request) {
	...
	switch {
	case strings.HasSuffix(handler.Filename, "csv"): //Handle file with extension CSV
		reader := csv.NewReader(file)
		record, readErr := reader.ReadAll()
		if readErr != nil {
			returnMessage = "Error while reading CSV file"
		} else {
	...
	case strings.HasSuffix(handler.Filename, "xlsx"): //Handle file with extension XLSX
		...
		xlFile, openFileErr := xlsx.OpenFile("./web/assets/files/" + handler.Filename)
		if openFileErr != nil {
			returnMessage = "Error while reading XLSX file"
		} else {
	...
	default:
		...
		returnMessage = "Upload file extension not supported"
	}

	returnStatus := false
	if returnMessage == "" {
		returnMessage = "File uploaded and information updated sucessfully"
		returnStatus = true
